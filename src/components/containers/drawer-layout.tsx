...
export default function DrawerLayout({
  children,
  links,
}: CustomComponentProps<{ links?: DrawerConfig }>) {
  return (
    <div className="h-full w-full grid grid-cols-[64px_1fr] md:grid-cols-[200px_1fr]">
      <aside className="h-full flex flex-col gap-4 bg-illuminating-emerald-300 dark:bg-zinc-600 text-zinc-900 dark:text-white">
        ...
        <div>{links?.userLinks?.map(createNavigationLink)}</div>
      </aside>
      <div
        className={`w-full h-full p-4 border-l-2 ${drawerLayoutContentColors} shadow-lg shadow-zinc-900`}
      >
        {children}
      </div>
    </div>
